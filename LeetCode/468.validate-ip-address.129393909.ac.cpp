...
public:
        ...
        for(int i = 0;i<ip.size();i++) {
            ...
            if (ip[i] == '.') {
                if (t.size()!=0){
                    ...
                    ipcontent.push_back(atoi(t.c_str()));
                    t.clear();
                } else 
                    return false;
            } 
    ...
    bool isIPv6(string ip) {
        ...
        for (int i=0;i < ip.size();i++) {
            if (ip[i]!=':') {
                if (hex.find(ip[i]) == std::string::npos) return false;
                t.push_back(ip[i]);
            } else {
                    if (!t.empty()) {
                    while (t.size()<4) {
        ...
        if (count+content.size() != 8) return false;
